...
export default function SalarySettings() {
  ...
  const handleSave = () => {
    ...
    if (!salary || salary < 1000 || salary > 100000) { toast.error('Base salary must be between ₪1,000 and ₪100,000'); return; }
    if (trans < 0 || trans > 5000) { toast.error('Transport allowance must be between ₪0 and ₪5,000'); return; }
    if (!rate || rate < 1 || rate > 500) { toast.error('Overtime rate must be between ₪1 and ₪500 per hour'); return; }
