...
import { ArrowLeft, Clock, Receipt, CheckCircle, XCircle, ChevronDown, ChevronUp } from "lucide-react";
    ...
    <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
      ...
      <div className="px-4 pb-4 flex gap-2">
        <Button
          ...
          onClick={() => onApprove(item.id, item.submitted_by, item.date)}
        ...
        <div className="max-w-lg mx-auto px-4 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">{totalPending} pending {totalPending === 1 ? 'entry' : 'entries'}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-6">
        ...
        <div>
          <h2 className="text-sm font-semibold text-slate-700 flex items-center gap-2 mb-3">
            ...
            Overtime Entries ({pendingSessions.length})
          </h2>
          {sessionsLoading ? (
            ...
            <div className="space-y-3">
              {pendingSessions.map(item => (
                <ReviewCard
                  ...
                  onApprove={(id, submitted_by, entry_date) =>
                    approveMutation.mutate({ entityType: 'OvertimeSession', id, submitted_by, entry_date })
                  }
                  onDecline={(id, submitted_by, entry_date, review_notes) =>
                    declineMutation.mutate({ entityType: 'OvertimeSession', id, submitted_by, entry_date, review_notes })
                  }
                />
              ))}
            </div>
            ...
            <div className="space-y-3">
              {pendingExpenses.map(item => (
                <ReviewCard
                  ...
                  isProcessing={isProcessing}
                  onApprove={(id, submitted_by, entry_date) =>
                    approveMutation.mutate({ entityType: 'Expense', id, submitted_by, entry_date })
                  }
                  onDecline={(id, submitted_by, entry_date, review_notes) =>
                    declineMutation.mutate({ entityType: 'Expense', id, submitted_by, entry_date, review_notes })
                  }
                />
              ))}
            </div>
